...
# segmentation
################
segrun <- function(binlogval, min.num.bin = 3) {
# segmentation module
# binlogval has three columns: chr, pos, log2fc
  if (nrow(binlogval) >= min.num.bin) {
    colnames(binlogval) <- c("Chr", "Pos", "log2fc")
    # segmentation:
    cna <-CNA(binlogval$log2fc, binlogval$Chr, binlogval$Pos, data.type ="logratio",
              sampleid = "segment")
    ...
    cna.seg <- segment(cna.smooth, alpha=0.005, nperm=10000, p.method="perm",
                       eta=0.005, min.width=min.num.bin, undo.splits = "sdundo", undo.SD = 3)
    # output:
    seg <- cna.seg$output
  } else {
    seg <- NA
...
# data separation
################
datasep <- function(binlogval, min.bin=3, max.distance=10000) {
# binlogval has three columns: chr, pos, log2fc
  colnames(binlogval) <- c("Chr", "Pos", "log2fc") 
}
```
...
low.bound <- max(low.bound, 20)   # minimum reads: at least 20
...
segrun(binlogval = log2fc, min.num.bin = 3)
