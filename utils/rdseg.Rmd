...
# determine xaxis
#################
mb_smartaxis <- function(maxnum) {
  options(scipen=999)
  numdigits <- nchar(maxnum)
  unit <- 10 ^ (numdigits - 1) / (2 - round((maxnum / 10 ^ numdigits), 0)) / 1000000 # 1 or 5 e (numdigits - 1)
  #unit <- 1
  subunit <- unit / 5 
  numsat <- unit * (0:10)
  numsat <- numsat[numsat < maxnum / 1000000]

  label.scale <- "Mb"
  numlabels <- numsat

  subunits <- seq(0, maxnum / 1000000, by = subunit)
...
if (adj0 == 1) {
	log2fc_stat <- table(round(bincount$log2fc, 1))
	deviation_from0 <- as.numeric(names(log2fc_stat)[which.max(log2fc_stat)])
	bincount$log2fc <- bincount$log2fc - deviation_from0
	if (abs(deviation_from0) > 1) {
		cat("Warning: the mode of logRD if fairly far from 0.")
		cat("\n")
		cat("All logRD values have been adjusted by substrating", deviation_from0);
		cat("\n")
	}
}
...
mtext(text = paste0("log2(", qry, ":", subj, ")"), side=2, line=3.2, cex=1.2,
      at = mean(c(0, num.chrs * offset - offset + 1)))

# x-axis
xaxis <- mb_smartaxis(bp_xmax)
axis(1, at=xaxis[[1]], labels=xaxis[[2]], tick=T, col="gray50", cex.axis=1.1)
mtext(paste0("Physical coordinate (", xaxis[[3]], ")"), side=1, line=3, cex=1.2)
abline(v=xaxis[[1]], col="grey85", cex=1)
abline(v=xaxis[[4]], col="grey95", cex=0.5)
