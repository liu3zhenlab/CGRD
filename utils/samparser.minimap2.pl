...
use constant CURRENT_VERSION => "0.03beta (2018.10.5)";
#use constant VERSION => "0.02beta (2011.6.12)";
use constant DEFAULT_MIN_IDENTICAL => 5000;       # Default minimum identical length per read
use constant DEFAULT_MAX_MISMATCHES => 10000;		# Default maximum mismatches per read
use constant DEFAULT_MAX_TAIL => 50000;				# Default maximum tail allowed
use constant DEFAULT_MAX_GAP => 10000; 			# Default maximum gap allowd
use constant INDEL_PENALTY => 1;
use constant DEFAULT_INSERT_MIN => 1000;         # Default minimal insert length
use constant DEFAULT_INSERT_MAX => 100000000;         # Default maximal insert length
...
while (<IN>) {
	...
	if (!/^@/) { # not header
		...
		## criterion: mismatch
			...
			# filter:
				if (scalar(@mismatches)==1 and $var_num<=$mismatches[0]) {
					$mmpass = 1;
				} elsif (scalar(@mismatches)==2 and ($var_num/$match_len)<=($mismatches[0]/$mismatches[1])) {
					$mmpass = 1;
				}
			} else {
...
Usage: perl samparser.pl -i [SAM file] [Options]
	...
	--keep_chimeric_paired: keep paired reads that are mapped to different chr or contigs if this is specified (on), default is off
