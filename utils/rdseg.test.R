...
setwd("~/scripts2/CGRD/test2/cgrdo/")
subj <- "B71"
qry <- "TF05-1"
bc <- "cgrdo_3_bin.counts"
libsizef <- "libsize.txt"
chrsizef <- "chrsize.txt"
outfile <- "segout.txt"
...
#bincountsub <- merge(bincountsub, qrycount[, c("Bin", qry)], by = "Bin")
...
seg2[seg2[, segmean.label] >= groupval[4], "Group"] <- paste0(qry, ">", subj) # up
seg2[seg2[, segmean.label] <= groupval[1], "Group"] <- paste0(qry, "<", subj) # dn
...
### output image:
plot.file <- paste0(qry, "_", subj, "cgrd.png")
par(mfrow=c(1,1))
offset <- 14
...
#png(plot.file, width=4, height=4, units = "in", pointsize = 4, res = 1200)
